...
namespace Shorokoo.Core.Nodes.AutoDiff
{
    internal static partial class AutoDiffs
    {
        ...
        internal static IValue?[] GridSampleGradient(IValue?[] inputs, IValue?[] outputGrads, OnnxCSharpAttributes attributes)
        {
            ...
            IValue ScatterCorner(IValue weight, IValue flatIdx)
            {
            ...
            IValue LookupCorner(IValue flatIdx, IValue mask)
            {
                var idx_flat = OnnxOp.Reshape(flatIdx,
                    OnnxOp.Concat([
                        OnnxOp.Reshape(N_s, Vector(1L), allowZero: false),

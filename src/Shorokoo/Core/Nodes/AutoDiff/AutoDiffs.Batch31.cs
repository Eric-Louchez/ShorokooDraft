...
namespace Shorokoo.Core.Nodes.AutoDiff
{
    ...
    internal static partial class AutoDiffs
    {
        ...
        [AutoDiff(SWISH)]
        public static IValue?[] Swish<T>(Tensor<T> x, Tensor<T> grad, float? alpha) where T : IVarType
        {
            var a = TypedConst(alpha ?? 1.0f, x);
            var one = TypedConst(1.0f, x);
            Tensor<T> s = (ImmutableTensor<T>)OnnxOp.Sigmoid(a * x);
        ...
        [AutoDiff(CUM_PROD)]
        public static IValue?[] CumProd<T1, T2>(Tensor<T1> x, Tensor<T2> axis, Tensor<T1> grad, bool? exclusive, bool? reverse)
            where T1 : IVarType
            where T2 : IVarType
        {
            var effectiveExclusive = exclusive ?? false;
            var effectiveReverse = reverse ?? false;
            Tensor<T1> y = (ImmutableTensor<T1>)OnnxOp.CumProd(x, axis, exclusive: effectiveExclusive, reverse: effectiveReverse);
            Tensor<T1> summed = (ImmutableTensor<T1>)OnnxOp.CumSum(grad * y, axis,
        ...
        public static IValue?[] RMSNormalization<T>(Tensor<T> x, Tensor<T> scale, Tensor<T> grad,
            long? axis, float? epsilon, long? stashType) where T : IVarType
        {
            ...
            Tensor<T> meanSq = (ImmutableTensor<T>)OnnxOp.ReduceMean(x * x, reduceAxes, keepdims: true);
            Tensor<T> invRms = (ImmutableTensor<T>)OnnxOp.Reciprocal((ImmutableTensor<T>)OnnxOp.Sqrt(meanSq + epsConst));
            var xHat = x * invRms;

            var gradScaled = grad * scale;
            Tensor<T> meanGradX = (ImmutableTensor<T>)OnnxOp.ReduceMean(gradScaled * x, reduceAxes, keepdims: true);
        ...
        internal static IValue?[] AttentionGradient(
        ...
        internal static IValue?[] RotaryEmbeddingGradient(
            IValue?[] inputs, IValue?[] outputGrads, OnnxCSharpAttributes attributes)
        {
            _ = inputs;
            _ = outputGrads;
        ...
        internal static IValue?[] TensorScatterGradient(
            IValue?[] inputs, IValue?[] outputGrads, OnnxCSharpAttributes attributes)
        {
            _ = inputs;
            _ = outputGrads;

...
namespace Shorokoo.Core.Nodes.AutoDiff
{
    ...
    internal static partial class AutoDiffs
    {
        ...
        public static IValue?[] Swish<T>(Tensor<T> x, Tensor<T> grad, float? alpha) where T : IVarType
        {
            ...
            Tensor<T> s = (ImmutableTensor)OnnxOp.Sigmoid(a * x);
            ...
            where T2 : IVarType
        {
            ...
            Tensor<T1> y = (ImmutableTensor)OnnxOp.CumProd(x, axis, exclusive: effectiveExclusive, reverse: effectiveReverse);
            Tensor<T1> summed = (ImmutableTensor)OnnxOp.CumSum(grad * y, axis,
            ...
            long? axis, float? epsilon, long? stashType) where T : IVarType
        {
            ...
            Tensor<T> meanSq = (ImmutableTensor)OnnxOp.ReduceMean(x * x, reduceAxes, keepdims: true);
            Tensor<T> invRms = (ImmutableTensor)OnnxOp.Reciprocal((ImmutableTensor)OnnxOp.Sqrt(meanSq + epsConst));
            var xHat = x * invRms;

            var gradScaled = grad * scale;
            Tensor<T> meanGradX = (ImmutableTensor)OnnxOp.ReduceMean(gradScaled * x, reduceAxes, keepdims: true);

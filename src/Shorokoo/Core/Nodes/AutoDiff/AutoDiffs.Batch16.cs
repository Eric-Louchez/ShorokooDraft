...
namespace Shorokoo.Core.Nodes.AutoDiff
{
    internal static partial class AutoDiffs
    {
            ...
            where T1 : IVarType
        {
            ...
            Tensor<T1> paddedGrad = (ImmutableTensor)OnnxOp.Pad(grad, padTensor, null, axes: null, mode: null);

            // Step 2: Get N, C from grad shape and compute spatial stride factors
            var gradShape = OnnxOp.Shape(grad);
            Tensor<int64> N_s = (ImmutableTensor)OnnxOp.Gather(gradShape, Scalar(0L), axis: 0);
            Tensor<int64> C_s = (ImmutableTensor)OnnxOp.Gather(gradShape, Scalar(1L), axis: 0);

            // Compute padded spatial dimensions
            var paddedDims = new IValue[nDims];
            for (int d = 0; d < nDims; d++)
            {
                Tensor<int64> imageDimD = (ImmutableTensor)OnnxOp.Gather(imageShape, Scalar((long)d), axis: 0);
                // Store the Immutable* graph value (not the boxed struct handle) so later reads can
                // downcast it back to ImmutableTensor.
                paddedDims[d] = (ImmutableTensor)(imageDimD + Scalar(effectivePads[d] + effectivePads[d + nDims]));
            }
            ...
            spatialStrides[nDims - 1] = (ImmutableScalar)Scalar(1L);
            for (int d = nDims - 2; d >= 0; d--)
                spatialStrides[d] = (ImmutableTensor)(((Tensor<int64>)(ImmutableTensor)spatialStrides[d + 1]) * ((Tensor<int64>)(ImmutableTensor)paddedDims[d + 1]));
            ...
            for (int d = 0; d < nDims; d++)
            {
                Tensor<int64> kd = (ImmutableTensor)OnnxOp.Gather(blockShape, Scalar((long)d), axis: 0);

                // Compute output dimension for this spatial dim:
                // od = (paddedDims[d] - dilations[d] * (kd - 1) - 1) / strides[d] + 1
                var od = ((ImmutableTensor)paddedDims[d] - Scalar(effectiveDilations[d]) * (kd - Scalar(1L)) - Scalar(1L)) / Scalar(effectiveStrides[d]) + Scalar(1L);

                // Create range tensors
                Tensor<int64> khRange = (ImmutableTensor)OnnxOp.Range(Scalar(0L), kd, Scalar(1L)); // [kd]
                Tensor<int64> ohRange = (ImmutableTensor)OnnxOp.Range(Scalar(0L), od, Scalar(1L)); // [od]
                ...
                Tensor<int64> khReshaped = (ImmutableTensor)OnnxOp.Reshape(khOffsets, Vector(khShape), allowZero: false);
                Tensor<int64> ohReshaped = (ImmutableTensor)OnnxOp.Reshape(ohStarts, Vector(ohShape), allowZero: false);

                // source_d = kh_offset + oh_start (broadcast to [..., kd, od, ...])
                var sourceD = khReshaped + ohReshaped;

                // Multiply by spatial stride and accumulate
                var contribution = sourceD * (ImmutableTensor)spatialStrides[d];
                flatIdx = flatIdx is null
                    ? (ImmutableTensor)contribution
                    : (ImmutableTensor)((Tensor<int64>)(ImmutableTensor)flatIdx + contribution);
            }
            ...
            flatIdx = (ImmutableTensor)OnnxOp.Transpose(flatIdx!, perm);

            // Reshape to [block_size, L]
            Tensor<int64> blockSize = (ImmutableTensor)OnnxOp.ReduceProd(blockShape, keepdims: false);
            var inputShapeT = OnnxOp.Shape(input);
            Tensor<int64> L = (ImmutableTensor)OnnxOp.Gather(inputShapeT, Scalar(2L), axis: 0);

            Tensor<int64> indexShape2D = (ImmutableTensor)OnnxOp.Concat([
                (ImmutableTensor)OnnxOp.Reshape(blockSize, Vector(1L), allowZero: false),
                (ImmutableTensor)OnnxOp.Reshape(L, Vector(1L), allowZero: false)
            ], axis: 0);
            flatIdx = (ImmutableTensor)OnnxOp.Reshape(flatIdx, indexShape2D, allowZero: false);
            ...
            Tensor<int64> spatialShape = (ImmutableTensor)OnnxOp.Slice(paddedGradShape, Vector(2L), Vector((long)(2 + nDims)));
            Tensor<int64> spatialFlat = (ImmutableTensor)OnnxOp.ReduceProd(spatialShape, keepdims: false);

            Tensor<int64> reshapeTo2D = (ImmutableTensor)OnnxOp.Concat([
                (ImmutableTensor)OnnxOp.Reshape(NC, Vector(1L), allowZero: false),
                (ImmutableTensor)OnnxOp.Reshape(spatialFlat, Vector(1L), allowZero: false)
            ], axis: 0);
            Tensor<T1> paddedFlat = (ImmutableTensor)OnnxOp.Reshape(paddedGrad, reshapeTo2D, allowZero: false);
            // paddedFlat: [N*C, spatial_flat]

            // Flatten index tensor to 1D: [block_size * L]
            Tensor<int64> flatIdxFlat = (ImmutableTensor)OnnxOp.Reshape(flatIdx, Vector(-1L), allowZero: false);

            // Expand indices to [N*C, block_size * L] for GatherElements
            var blockTimesL = blockSize * L;
            Tensor<int64> expandShape = (ImmutableTensor)OnnxOp.Concat([
                (ImmutableTensor)OnnxOp.Reshape(NC, Vector(1L), allowZero: false),
                (ImmutableTensor)OnnxOp.Reshape(blockTimesL, Vector(1L), allowZero: false)
            ], axis: 0);
            Tensor<int64> flatIdxExpanded = (ImmutableTensor)OnnxOp.Expand(
                (ImmutableTensor)OnnxOp.Unsqueeze(flatIdxFlat, Vector(0L)),
                expandShape);
            // flatIdxExpanded: [N*C, block_size * L]

            // GatherElements axis=1: output[i,j] = paddedFlat[i, flatIdxExpanded[i,j]]
            Tensor<T1> gathered = (ImmutableTensor)OnnxOp.GatherElements(paddedFlat, flatIdxExpanded, axis: 1);
            // gathered: [N*C, block_size * L]

            // Step 5: Reshape to [N, C * block_size, L] to match input shape
            var CTimesB = C_s * blockSize;
            Tensor<int64> outputShape = (ImmutableTensor)OnnxOp.Concat([
                (ImmutableTensor)OnnxOp.Reshape(N_s, Vector(1L), allowZero: false),
                (ImmutableTensor)OnnxOp.Reshape(CTimesB, Vector(1L), allowZero: false),
                (ImmutableTensor)OnnxOp.Reshape(L, Vector(1L), allowZero: false)
            ], axis: 0);
            Tensor<T1> dInput = (ImmutableTensor)OnnxOp.Reshape(gathered, outputShape, allowZero: false);

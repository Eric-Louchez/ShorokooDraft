...
namespace Shorokoo.Core.Nodes.AutoDiff
{
    internal static partial class AutoDiffs
    {
            ...
            where T1 : IVarType
        {
            ...
            Tensor<T1> paddedGrad = (Variable)OnnxOp.Pad(grad, padTensor, null, axes: null, mode: null);

            // Step 2: Get N, C from grad shape and compute spatial stride factors
            var gradShape = OnnxOp.Shape(grad);
            Tensor<int64> N_s = (Variable)OnnxOp.Gather(gradShape, Scalar(0L), axis: 0);
            Tensor<int64> C_s = (Variable)OnnxOp.Gather(gradShape, Scalar(1L), axis: 0);

            // Compute padded spatial dimensions
            var paddedDims = new IValue[nDims];
            for (int d = 0; d < nDims; d++)
            {
                Tensor<int64> imageDimD = (Variable)OnnxOp.Gather(imageShape, Scalar((long)d), axis: 0);
            ...
            for (int d = 0; d < nDims; d++)
            {
                Tensor<int64> kd = (Variable)OnnxOp.Gather(blockShape, Scalar((long)d), axis: 0);
                ...
                Tensor<int64> khRange = (Variable)OnnxOp.Range(Scalar(0L), kd, Scalar(1L)); // [kd]
                Tensor<int64> ohRange = (Variable)OnnxOp.Range(Scalar(0L), od, Scalar(1L)); // [od]
                ...
                Tensor<int64> khReshaped = (Variable)OnnxOp.Reshape(khOffsets, Vector(khShape), allowZero: false);
                Tensor<int64> ohReshaped = (Variable)OnnxOp.Reshape(ohStarts, Vector(ohShape), allowZero: false);
            ...
            flatIdx = (Variable)OnnxOp.Transpose(flatIdx!, perm);

            // Reshape to [block_size, L]
            Tensor<int64> blockSize = (Variable)OnnxOp.ReduceProd(blockShape, keepdims: false);
            var inputShapeT = OnnxOp.Shape(input);
            Tensor<int64> L = (Variable)OnnxOp.Gather(inputShapeT, Scalar(2L), axis: 0);

            Tensor<int64> indexShape2D = (Variable)OnnxOp.Concat([
                (Variable)OnnxOp.Reshape(blockSize, Vector(1L), allowZero: false),
                (Variable)OnnxOp.Reshape(L, Vector(1L), allowZero: false)
            ], axis: 0);
            flatIdx = (Variable)OnnxOp.Reshape(flatIdx, indexShape2D, allowZero: false);
            ...
            Tensor<int64> spatialShape = (Variable)OnnxOp.Slice(paddedGradShape, Vector(2L), Vector((long)(2 + nDims)));
            Tensor<int64> spatialFlat = (Variable)OnnxOp.ReduceProd(spatialShape, keepdims: false);

            Tensor<int64> reshapeTo2D = (Variable)OnnxOp.Concat([
                (Variable)OnnxOp.Reshape(NC, Vector(1L), allowZero: false),
                (Variable)OnnxOp.Reshape(spatialFlat, Vector(1L), allowZero: false)
            ], axis: 0);
            Tensor<T1> paddedFlat = (Variable)OnnxOp.Reshape(paddedGrad, reshapeTo2D, allowZero: false);
            // paddedFlat: [N*C, spatial_flat]

            // Flatten index tensor to 1D: [block_size * L]
            Tensor<int64> flatIdxFlat = (Variable)OnnxOp.Reshape(flatIdx, Vector(-1L), allowZero: false);

            // Expand indices to [N*C, block_size * L] for GatherElements
            var blockTimesL = blockSize * L;
            Tensor<int64> expandShape = (Variable)OnnxOp.Concat([
                (Variable)OnnxOp.Reshape(NC, Vector(1L), allowZero: false),
                (Variable)OnnxOp.Reshape(blockTimesL, Vector(1L), allowZero: false)
            ], axis: 0);
            Tensor<int64> flatIdxExpanded = (Variable)OnnxOp.Expand(
                (Variable)OnnxOp.Unsqueeze(flatIdxFlat, Vector(0L)),
                expandShape);
            // flatIdxExpanded: [N*C, block_size * L]

            // GatherElements axis=1: output[i,j] = paddedFlat[i, flatIdxExpanded[i,j]]
            Tensor<T1> gathered = (Variable)OnnxOp.GatherElements(paddedFlat, flatIdxExpanded, axis: 1);
            // gathered: [N*C, block_size * L]

            // Step 5: Reshape to [N, C * block_size, L] to match input shape
            var CTimesB = C_s * blockSize;
            Tensor<int64> outputShape = (Variable)OnnxOp.Concat([
                (Variable)OnnxOp.Reshape(N_s, Vector(1L), allowZero: false),
                (Variable)OnnxOp.Reshape(CTimesB, Vector(1L), allowZero: false),
                (Variable)OnnxOp.Reshape(L, Vector(1L), allowZero: false)
            ], axis: 0);
            Tensor<T1> dInput = (Variable)OnnxOp.Reshape(gathered, outputShape, allowZero: false);

...
namespace Shorokoo.Graph
{
    ...
    /// the IValue / Node view on every access.
    /// </summary>
    public partial class FastComputationGraph
    {
        ...
        public ImmutableArray<IValue> InputTensors
            => FastComputationGraphConverter.BuildNodes(this).inputs;

        public ImmutableArray<IValue> OutputTensors
    ...
    public static class FastComputationGraphCompat
    {
        ...
        /// used to read .Length on the IValue[] form of Inputs/Outputs.
        /// </summary>
        public static int Length(this System.Collections.Generic.List<FastTensorKey> list) => list.Count;

        public static ImmutableArray<IValue> GetHyperparamInputs(this FastComputationGraph graph)
            => graph.InputTensors
                .Where(x => x.OwningNode.Attributes.GetEnumVal<InputType>(OnnxOpAttributeNames.ShrkAttrInputType) == InputType.Hyperparam)
                .ToImmutableArray();

        public static ImmutableArray<IValue> GetNonHyperparamInputs(this FastComputationGraph graph)
            => graph.InputTensors
                .Where(x => x.OwningNode.Attributes.GetEnumVal<InputType>(OnnxOpAttributeNames.ShrkAttrInputType) != InputType.Hyperparam)
                .ToImmutableArray();
        ...
        /// (one IValue per output slot). Mirrors the deleted CG property.
        /// </summary>
        public static ImmutableArray<IValue> TopologicalOrderTensors(this FastComputationGraph graph)

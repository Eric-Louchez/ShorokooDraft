...
namespace Shorokoo
{
         ...
         where T : IVarType
    {
        ...
        public virtual Vector<int64> DShape =>
            (ImmutableVector<int64>)OnnxOp.Shape(this, null, null);
        ...
        {
            if (this is ImmutableVector<T> vec)
            {
                return vec;
            }
            else
            {
                return (ImmutableVector<T>)OnnxOp.Identity(this, rank: 1);
            }
        }
        ...
        {
            if (this is ImmutableScalar<T> scalar)
            {
                return scalar;
            }
            else
            {
                return (ImmutableScalar<T>)OnnxOp.Identity(this, rank: 0);
            }
        }
...
/// <summary>Casts the element type to <typeparamref name="V"/>; returns this tensor unchanged when the types already match.</summary>
    ...
    /// <see cref="ImmutableTensor{T}"/> directly (value-copy semantics for the Module DSL) and
...
#pragma warning restore CS0618
        ...
        public Scalar<T> Scalar() => Imm.Scalar();
        ...
        public static Tensor<T> operator -(Tensor<T> left, Tensor<T> right)
        {
            return (ImmutableTensor<T>)OnnxOp.Sub(left, right);
        }

        /// <summary>Element-wise multiplication.</summary>
        public static Tensor<T> operator *(Tensor<T> left, Tensor<T> right)
        {
            return (ImmutableTensor<T>)OnnxOp.Mul(left, right);
        }

        /// <summary>Element-wise division.</summary>
        public static Tensor<T> operator /(Tensor<T> left, Tensor<T> right)
        {
            return (ImmutableTensor<T>)OnnxOp.Div(left, right);
        }

        /// <summary>Element-wise modulo.</summary>
        public static Tensor<T> operator %(Tensor<T> left, Tensor<T> right)
            => (ImmutableTensor<T>)OnnxOp.Mod(left, right);

        /// <summary>Element-wise XOR: logical for bit tensors, bitwise otherwise.</summary>
        public static Tensor<T> operator ^(Tensor<T> left, Tensor<T> right)
            => typeof(T) == typeof(bit) ?
                (ImmutableTensor<T>)OnnxOp.Xor(left, right) :
                (ImmutableTensor<T>)OnnxOp.BitwiseXor(left, right);

        /// <summary>Element-wise AND: logical for bit tensors, bitwise otherwise.</summary>
        public static Tensor<T> operator &(Tensor<T> left, Tensor<T> right)
            => typeof(T) == typeof(bit) ?
                (ImmutableTensor<T>)OnnxOp.And(left, right) :
                (ImmutableTensor<T>)OnnxOp.BitwiseAnd(left, right);

        /// <summary>Element-wise OR: logical for bit tensors, bitwise otherwise.</summary>
        public static Tensor<T> operator |(Tensor<T> left, Tensor<T> right)
            => typeof(T) == typeof(bit) ?
                (ImmutableTensor<T>)OnnxOp.Or(left, right) :
                (ImmutableTensor<T>)OnnxOp.BitwiseOr(left, right);

        /// <summary>Element-wise left bit-shift.</summary>
        public static Tensor<T> operator <<(Tensor<T> left, Tensor<T> right)
            => (ImmutableTensor<T>)OnnxOp.BitShift(left, right, BitShiftDirection.Left);

        /// <summary>Element-wise right bit-shift.</summary>
        public static Tensor<T> operator >>(Tensor<T> left, Tensor<T> right)
            => (ImmutableTensor<T>)OnnxOp.BitShift(left, right, BitShiftDirection.Right);

        /// <summary>Element-wise negation.</summary>
        public static Tensor<T> operator -(Tensor<T> input)
        {
            return (ImmutableTensor<T>)OnnxOp.Neg(input);
        }

        /// <summary>Element-wise NOT: logical for bit tensors, bitwise otherwise.</summary>
        public static Tensor<T> operator !(Tensor<T> input)
            => typeof(T) == typeof(bit) ?
                (ImmutableTensor<T>)OnnxOp.Not(input) :
                (ImmutableTensor<T>)OnnxOp.BitwiseNot(input);

        /// <summary>Element-wise greater-than, yielding a bit tensor.</summary>
        public static Tensor<bit> operator >(Tensor<T> left, Tensor<T> right) => (ImmutableTensor<bit>)OnnxOp.Greater(left, right);

        /// <summary>Element-wise greater-or-equal, yielding a bit tensor.</summary>
        public static Tensor<bit> operator >=(Tensor<T> left, Tensor<T> right) => (ImmutableTensor<bit>)OnnxOp.GreaterOrEqual(left, right);

        /// <summary>Element-wise less-than, yielding a bit tensor.</summary>
        public static Tensor<bit> operator <(Tensor<T> left, Tensor<T> right) => (ImmutableTensor<bit>)OnnxOp.Less(left, right);

        /// <summary>Element-wise less-or-equal, yielding a bit tensor.</summary>
        public static Tensor<bit> operator <=(Tensor<T> left, Tensor<T> right) => (ImmutableTensor<bit>)OnnxOp.LessOrEqual(left, right);

        /// <summary>Element-wise equality, yielding a bit tensor (not reference equality).</summary>
        public static Tensor<bit> operator ==(Tensor<T> left, Tensor<T> right) => (ImmutableTensor<bit>)OnnxOp.Equal(left, right);
        ...
        public static Tensor<T> Fill(Vector<int64> shape, TensorData val)
        {
            ...
            return (ImmutableTensor<T>)OnnxOp.ConstantOfShape(shape, val);
        }
        ...
        public Tensor<T> Bernoulli(float? seed = null)
            => (ImmutableTensor<T>)OnnxOp.Bernoulli(this, null, seed);
        ...
        public Tensor<V> Bernoulli<V>(float? seed = null) where V : CommonLike
            => (ImmutableTensor<V>)OnnxOp.Bernoulli(this, OnnxUtils.GetDType<V>(), seed);

        /// <summary>Element-wise ceiling.</summary>
        public Tensor<T> Ceiling()
            => (ImmutableTensor<T>)OnnxOp.Ceil(this);

        /// <summary>Element-wise CELU activation.</summary>
        public Tensor<T> Celu(float alpha = 1.0f)
            => (ImmutableTensor<T>)OnnxOp.Celu(this, alpha == 1.0f ? null : alpha);
        ...
        public Tensor<T> Cos()
            => (ImmutableTensor<T>)OnnxOp.Cos(this);

        /// <summary>Element-wise hyperbolic cosine.</summary>
        public Tensor<T> Cosh()
            => (ImmutableTensor<T>)OnnxOp.Cosh(this);
        ...
        public Tensor<T> Sigmoid()
            => (ImmutableTensor<T>)OnnxOp.Sigmoid(this);
        ...
        public Tensor<T> HardSigmoid(float? alpha = null, float? beta = null)
            => (ImmutableTensor<T>)OnnxOp.HardSigmoid(this, alpha, beta);

        /// <summary>Element-wise hard swish.</summary>
        public Tensor<T> HardSwish()
            => (ImmutableTensor<T>)OnnxOp.HardSwish(this);
        ...
        public Tensor<bit> IsInf(bool detectNegative = true, bool detectPositive = true)
            => (ImmutableTensor<bit>)OnnxOp.IsInf(this,
        ...
        public Tensor<bit> IsNaN()
            => (ImmutableTensor<bit>)OnnxOp.IsNaN(this);
        ...
        public Tensor<T> Clip(Scalar<T> min, Scalar<T> max)
            => (ImmutableTensor<T>)OnnxOp.Clip(this, min, max);
        ...
        public Tensor<T> Exp()
            => (ImmutableTensor<T>)OnnxOp.Exp(this);

...
public partial class NAdamOptimizer
{
        ...
        [Hyper(0.004f)] Scalar<float32> momentumDecay)
    {
        ...
        var muT = beta1 * (one - half * (Scalar<float32>)(ImmutableScalar<float32>)OnnxOp.Pow(base96, newStep * momentumDecay));
        var muNext = beta1 * (one - half * (Scalar<float32>)(ImmutableScalar<float32>)OnnxOp.Pow(base96, (newStep + one) * momentumDecay));

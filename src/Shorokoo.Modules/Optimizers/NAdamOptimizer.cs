...
public partial class NAdamOptimizer
{
        ...
        [Hyper(0.004f)] Scalar<float32> momentumDecay)
    {
        ...
        var muT = beta1 * (one - half * (Scalar<float32>)(Variable)OnnxOp.Pow(base96, newStep * momentumDecay));
        var muNext = beta1 * (one - half * (Scalar<float32>)(Variable)OnnxOp.Pow(base96, (newStep + one) * momentumDecay));
        ...
        var vHat = newV / (one - (Tensor<float32>)(Variable)OnnxOp.Pow(beta2, newStep));

...
public static class LRNHelper
{
    ...
    public static Tensor<T> Lrn<T>(Tensor<T> x, long size = 5,
        ...
        => (Tensor<T>)(ImmutableTensor<T>)OnnxOp.Lrn(x, alpha, beta, bias: k, size: size);   // k → ONNX 'bias'
}
...
public partial class LocalResponseNorm
{
        ...
        [Hyper(1.0f)] Scalar<float32> k)         // PyTorch 'k' / ONNX 'bias' (additive constant)
    {
        ...
        var cDim = (Tensor<int64>)(ImmutableTensor<int64>)OnnxOp.Slice(OnnxOp.Shape(x), Vector(1L), Vector(2L));
        ...
        Tensor<float32> ChannelWindowSum(Tensor<float32> t)
        {
            ...
            var padded = (Tensor<float32>)(ImmutableTensor<float32>)OnnxOp.Pad(t, pads, null, axes: Vector(1L), mode: PadMode.Constant);

            var result = (Tensor<float32>)(ImmutableTensor<float32>)OnnxOp.Slice(padded, Vector(0L), cDim, Vector(1L));
            for (long i = 1; i < size; i++)
            {
                var start = Vector(i);
                var end = (Tensor<int64>)(ImmutableTensor<int64>)OnnxOp.Add(cDim, Vector(i));
                result = result + (Tensor<float32>)(ImmutableTensor<float32>)OnnxOp.Slice(padded, start, end, Vector(1L));
            }
            return result;
        }
        ...
        // y = x · pool^(−β).
        return x * (Tensor<float32>)(ImmutableTensor<float32>)OnnxOp.Pow(pool, -beta);
    }
}

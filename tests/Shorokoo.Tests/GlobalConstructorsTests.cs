...
/// <see cref="Globals.TensorStruct{T}(IVariable[])"/>, the DispatchProxy variant that mirrors
...
public partial class VariableAndInputConstructorsModel
{
    public static Scalar<bit> Inline(Tensor<float32> input)
    {
        ...
        Assert.NotNull(((IVariable)Scalar(1.0f)).ToInput());
        Assert.NotNull(((IVariable)OptionalTensor<float32>(Vector(1.0f, 2.0f).Reshape(Vector(2L)))).ToInput());
        Assert.NotNull(((IVariable)TensorSequence(DType.Float32, (ITensor)Vector(1.0f))).ToInput());

        Assert.NotNull(TensorSequence(DType.Float32));
        Assert.NotNull(TensorSequence(DType.Float32, (ITensor)Vector(1.0f, 2.0f)));
        Assert.NotNull(OptionalTensor(DType.Float32));
        Assert.NotNull(OptionalTensor(DType.Float32, (ITensor)Vector(1.0f)));

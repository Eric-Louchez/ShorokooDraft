...
public partial class NNLocalResponseNormClosedForm
{
    public static Scalar<bit> Inline(Tensor<float32> x)   // [1, 5, 2, 2]
    {
        ...
        var x2 = x * x;
        var padded = (Tensor<float32>)(Variable)OnnxOp.Pad(x2, Vector(2L, 2L), null,
            axes: Vector(1L), mode: PadMode.Constant);
        var sum = (Tensor<float32>)(Variable)OnnxOp.Slice(padded, Vector(0L), Vector(5L), Vector(1L));
        for (long i = 1; i < 5; i++)
            sum = sum + (Tensor<float32>)(Variable)OnnxOp.Slice(padded, Vector(i), Vector(5L + i), Vector(1L));
        var pool = k + (a / Scalar(5f)) * sum;
        var yRef = x * (Tensor<float32>)(Variable)OnnxOp.Pow(pool, -b);
...
public partial class NNLrnHelperArbitrarySizeClosedForm
{
    public static Scalar<bit> Inline(Tensor<float32> x)   // [1, 5, 2, 2]
    {
        ...
        var x2 = x * x;
        var padded = (Tensor<float32>)(Variable)OnnxOp.Pad(x2, Vector(leftHalf, rightHalf), null,
            axes: Vector(1L), mode: PadMode.Constant);
        const long cDim = 5L;   // C = 5 (known here)
        var sum = (Tensor<float32>)(Variable)OnnxOp.Slice(padded, Vector(0L), Vector(cDim), Vector(1L));
        for (long i = 1; i < size; i++)
            sum = sum + (Tensor<float32>)(Variable)OnnxOp.Slice(padded, Vector(i), Vector(cDim + i), Vector(1L));
        var pool = Scalar(k) + (Scalar(a) / Scalar((float)size)) * sum;
        var yRef = x * (Tensor<float32>)(Variable)OnnxOp.Pow(pool, Scalar(-b));
